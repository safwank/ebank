...
-export([start/0, start_link/0, stop/0, 
		 ...
		 withdraw/3, transfer/4, block/1, eject/1
		]).
...
handle_call({account, Accounts}, _, State) ->
	...
	{reply, Reply, State};
handle_call({is_pin_valid, AccountNumber, Pin}, _From = {Pid, _}, State) ->
	case {lists:member(AccountNumber, State#state.blocked),
		  lists:member(AccountNumber, State#state.alarms)} of
		{true, true} ->
			{reply, true, State};
		{true, false} ->
			alarm_handler:set_alarm({AccountNumber, Pid}),
			{reply, true, State#state{alarms = [AccountNumber | State#state.alarms]}};
		{false, _} ->
			Account = find_account(AccountNumber, State),
			IsPinValid = is_pin_valid(Account, Pin),
			stats:log({login, AccountNumber, IsPinValid}),
			{reply, IsPinValid, State}
	end;
...
eject(AccountNo, State = #state{alarms=Alarms}) ->
	case lists:member(AccountNo, Alarms) of
		true ->
			alarm_handler:clear_alarm(AccountNo),
			State#state{alarms = lists:delete(AccountNo,
											  State#state.alarms)};
		false -> State
	end.
